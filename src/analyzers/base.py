...
class ExtensionInfo:
    ...
    platform_availability: Optional[Dict[str, Dict]] = (
        None  # platform -> {available, date, error}
    )
    availability_date: Optional[datetime] = (
        None  # Earliest availability date across platforms
    )
    available_platforms: Optional[List[str]] = (
        None  # List of platforms where extension is available
    )
