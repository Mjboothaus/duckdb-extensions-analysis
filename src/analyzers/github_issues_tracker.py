...
class GitHubIssuesTracker:
    ...
    async def fetch_extension_issues_from_repos(
        self,
        ...
        """
        logger.info(
            f"Fetching GitHub issues directly from {len(extension_repos)} extension repositories"
        )
            ...
            for ext_name, repo_path in extension_repos.items():
                try:
                    ...
                    issues = await self._fetch_repository_issues(
                        client, repo_path, since_date, include_closed
                    )
                    ...
                    logger.warning(
                        f"Failed to fetch issues for {ext_name} from {repo_path}: {e}"
                    )
                    continue
    ...
    async def fetch_extension_issues(
        self,
        ...
        if extension_repos:
            return await self.fetch_extension_issues_from_repos(
                extension_repos, days_back, include_closed
            )

        # Fallback to the original search approach for extensions without known repositories
        logger.info(
            "No known repositories found, falling back to search-based approach"
        )
        return await self._fetch_extension_issues_fallback(
            extension_names, days_back, include_closed
        )
    ...
    async def _fetch_repository_issues(
        ...
        # Fetch open issues
        open_issues = await self._fetch_issues_by_state(
            client, repo_path, "open", since_date
        )
        issues.extend(open_issues)

        # Fetch closed issues if requested
        if include_closed:
            closed_issues = await self._fetch_issues_by_state(
                client, repo_path, "closed", since_date
            )
            issues.extend(closed_issues)

        return issues

    async def _fetch_issues_by_state(
        self,
        client: httpx.AsyncClient,
        repo_path: str,
        state: str,
                ...
                try:
                    ...
                    issue = ExtensionIssue(
                        ...
                        updated_at=datetime.fromisoformat(
                            item["updated_at"].rstrip("Z")
                        ),
                        closed_at=datetime.fromisoformat(item["closed_at"].rstrip("Z"))
                    ...
                    logger.warning(
                        f"Failed to parse issue {item.get('number', 'unknown')}: {e}"
                    )
                    continue
    ...
    async def _fetch_extension_issues_fallback(
        self,
        extension_names: List[str],
        days_back: int = 90,
        include_closed: bool = True,
        ...
        for ext_name in extension_names[
            :10
        ]:  # Limit to first 10 extensions to avoid rate limits
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            for i, query in enumerate(all_queries):
                try:
                    logger.debug(
                        f"Searching issues with query {i + 1}/{len(all_queries)}: {query}"
                    )
                    issues = await self._search_issues(
                        client, query, days_back, include_closed
                    )
        ...
        logger.info(
            f"Found {len(extension_issues)} extension-related issues using fallback method"
        )
    ...
    async def _search_issues(
        ...
        search_query = (
            f"repo:{self.repo_owner}/{self.repo_name} {query} created:>={since_date}"
        )
    ...
    def _extract_mentioned_extensions(
        self, issue: ExtensionIssue, known_extensions: Set[str]
    ) -> Set[str]:
    ...
    async def _make_api_request(
        self, client: httpx.AsyncClient, url: str, params: dict, headers: dict
    ) -> httpx.Response:
        ...
        if response.status_code == 403:
            ...
            if (
                ...
                logger.warning(
                    f"GitHub API rate limit hit. Remaining: {remaining}, Reset: {reset_time}"
                )
                ...
                logger.warning(
                    f"GitHub API returned 403 Forbidden: {response.text[:200]}..."
                )
    ...
    def _determine_severity(self, issue: ExtensionIssue) -> str:
        ...
        for label in issue.labels:
            ...
            if any(
                word in label_lower for word in ["critical", "urgent", "bug", "high"]
            ):
                return "high"
    ...
    def get_extension_issues_summary(
        self, issues: List[ExtensionIssue], extension_name: str
    ) -> Dict[str, Any]:
        """Get a summary of issues for a specific extension."""
        ext_issues = [
            issue for issue in issues if extension_name in issue.extension_names
        ]
        ...
        recent_issues = [
            issue for issue in ext_issues if issue.created_at >= recent_cutoff
        ]
        ...
        return {
            ...
            "latest_issues": sorted(
                ext_issues, key=lambda x: x.updated_at, reverse=True
            )[:5],
        }
