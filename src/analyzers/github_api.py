...
class GitHubAPIClient:
    ...
    @retry(
        stop=stop_after_attempt(
            3
        ),  # Reduced from 5 to avoid making abuse detection worse
        wait=wait_exponential(
            multiplier=3, min=5, max=120
        ),  # Longer waits: 5s, 15s, 45s
        retry=lambda retry_state: retry_state.outcome.failed
        and (
            retry_state.attempt_number == 1
            or (
                hasattr(retry_state.outcome.exception(), "response")
                and retry_state.outcome.exception().response.status_code
        ...
        async with self.rate_limiter:
            ...
            try:
                response = await client.get(
                    url, headers=request_headers, timeout=10, follow_redirects=True
                )

                # Handle 304 Not Modified - content hasn't changed
                if response.status_code == 304 and cached_data:
                    logger.info("✓ Content unchanged (304), using cached data")
                    cached_time, data = cached_data
                    if isinstance(data, dict) and "_etag" in data:
                        data_copy = data.copy()
                        del data_copy["_etag"]
                ...
                if e.response.status_code in (403, 429):
                    ...
                    logger.error(f"\n{'=' * 60}")
                    logger.error(
                        f"GitHub API Error {e.response.status_code} for: {url_path}"
                    )
                    ...
                    logger.error(
                        f"  X-RateLimit-Limit: {e.response.headers.get('x-ratelimit-limit', 'N/A')}"
                    )
                    logger.error(
                        f"  X-RateLimit-Remaining: {e.response.headers.get('x-ratelimit-remaining', 'N/A')}"
                    )
                    logger.error(
                        f"  X-RateLimit-Reset: {e.response.headers.get('x-ratelimit-reset', 'N/A')}"
                    )
                    logger.error(
                        f"  X-RateLimit-Resource: {e.response.headers.get('x-ratelimit-resource', 'N/A')}"
                    )
                    logger.error(
                        f"  X-RateLimit-Used: {e.response.headers.get('x-ratelimit-used', 'N/A')}"
                    )
                    logger.error(
                        f"  Retry-After: {e.response.headers.get('retry-after', 'N/A')}"
                    )
                    logger.error(
                        f"  X-GitHub-Request-Id: {e.response.headers.get('x-github-request-id', 'N/A')}"
                    )
                    logger.error(f"{'=' * 60}\n")
                    ...
                    if retry_after:
                        ...
                        logger.warning(
                            f"Waiting {wait_time}s as specified by Retry-After header"
                        )
                        ...
                        logger.warning("No Retry-After header, using default 15s wait")
                        await asyncio.sleep(15)
                raise

    async def get_repository_info(
        self, client: httpx.AsyncClient, repo_path: str
    ) -> Optional[Dict]:
                    ...
                    if processed_release["published_at"]:
                        try:
                            pub_date = datetime.fromisoformat(
                                processed_release["published_at"].replace("Z", "+00:00")
                            )
                            processed_release["published_date"] = pub_date
                            processed_release["days_ago"] = (
                                datetime.now().replace(tzinfo=pub_date.tzinfo)
                                - pub_date
                            ).days
                        except Exception:
