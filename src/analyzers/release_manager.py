...
class DuckDBReleaseManager:
    ...
    GITHUB_RELEASES_API_URL = "https://api.github.com/repos/duckdb/duckdb/releases"
    CACHE_KEY = "duckdb_releases_csv"
    CACHE_KEY_UPCOMING = "duckdb_releases_upcoming"
    CACHE_KEY_GITHUB_RELEASES = "duckdb_github_releases"
    ...
    def _fetch_releases_csv(self) -> str:
        ...
        if cached_data:
            ...
            if age_hours < self.cache_hours:
                logger.debug(
                    f"Using cached DuckDB releases CSV (age: {age_hours:.1f}h)"
                )
                return csv_content
        ...
        try:
            ...
            response = httpx.get(
                self.RELEASES_CSV_URL, timeout=timeout, follow_redirects=True
            )
        ...
        for row in csv_reader:
            try:
                ...
                release = DuckDBRelease(
                    ...
                    duck_species_primary=row.get("duck_species_primary", "").strip()
                    or None,
                    duck_species_secondary=row.get("duck_species_secondary", "").strip()
                    or None,
                ...
                logger.debug(
                    f"Parsed release: {release.version} ({release.release_date})"
                )
    ...
    def _fetch_upcoming_releases(self) -> List[DuckDBRelease]:
        ...
        try:
            ...
            timeout = getattr(self.config, "timeout_seconds", 10)
            response = httpx.get(
                self.RELEASE_CALENDAR_URL, timeout=timeout, follow_redirects=True
            )
            response.raise_for_status()
            ...
            pattern = (
                r"<td[^>]*>(\d{4}-\d{2}-\d{2})</td>\s*<td[^>]*>(\d+\.\d+\.\d+)</td>"
            )
            ...
            for date_str, version_str in matches:
                try:
                    ...
                    if release_date and release_date > date.today():
                        ...
                        logger.info(
                            f"Found upcoming release: {version_str} on {date_str}"
                        )
                except Exception as e:
                    logger.warning(
                        f"Failed to parse upcoming release {version_str}: {e}"
                    )
                    continue
    ...
    def _fetch_github_releases(self) -> List[DuckDBRelease]:
        """Fetch recent releases from GitHub Releases API to catch newly released versions."""
        # Check cache first
        cached_data = self.cache.get(self.CACHE_KEY_GITHUB_RELEASES)
        if cached_data:
            cached_time, github_releases = cached_data
            age_hours = (datetime.now() - cached_time).total_seconds() / 3600
            if age_hours < self.cache_hours:
                logger.debug(f"Using cached GitHub releases (age: {age_hours:.1f}h)")
                return github_releases

        logger.info("Fetching recent releases from GitHub Releases API")
        github_releases: List[DuckDBRelease] = []
        ...
        try:
            ...
            for release in releases_data:
                # Ignore drafts and pre-releases for date purposes.
            ...
            # Cache the results
            self.cache.set(
                self.CACHE_KEY_GITHUB_RELEASES, (datetime.now(), github_releases)
            )
            logger.info(
                f"Found {len(github_releases)} releases from GitHub releases endpoint"
            )
            ...
            # Return stale cache if available
            if cached_data:
                logger.warning("Using stale cached GitHub releases")
                return cached_data[1]
    ...
    def load_releases(self) -> List[DuckDBRelease]:
        ...
        # Fetch GitHub releases (for recently released versions not yet in CSV)
        github_releases = self._fetch_github_releases()
        ...
        new_github = [
            r for r in github_releases if r.version.lstrip("v") not in existing_versions
        ]
        ...
        logger.info(
            f"Total releases: {len(all_releases)} (CSV: {len(csv_releases)}, GitHub: {len(new_github)}, Upcoming: {len(new_upcoming)})"
        )
    ...
    def get_latest_lts_release(self) -> Optional[DuckDBRelease]:
        ...
        logger.info(
            f"Latest LTS release: {latest_lts.version} ({latest_lts.release_date})"
        )
        return latest_lts
            ...
            report_data.append(
                {
                    ...
                    "duck_species": release.duck_species_secondary
                    or release.duck_species_primary
                    or "–",
                    "lts": "✓" if release.lts else "",
                    "blog_post": release.blog_post or "",
                    "end_of_life": release.end_of_life.strftime("%Y-%m-%d")
                    if release.end_of_life
                    else "",
                    "is_future": release.is_future,
                    "status": "Planned"
                    if release.is_future
                    else ("Active" if release.is_active else "EOL"),
                }
            )
