...
class AnalysisOrchestrator:
    ...
    def __init__(
        ...
        self.core_analyzer = CoreExtensionAnalyzer(
            config, self.github_client, cache_hours
        )
        self.community_analyzer = CommunityExtensionAnalyzer(
            config, self.github_client, cache_hours, enable_compatibility_testing
        )
        self.database_manager = DatabaseManager(config)
        self.report_generator = ReportGenerator(config)
        self.github_issues_tracker = GitHubIssuesTracker(
            self.github_client, cache_hours
        )
        self.url_validator = URLValidator(timeout=10)

    async def analyze_core_extensions(
        ...
        if duckdb_version:
            ...
            return await self.core_analyzer.analyze_with_platform_availability(
                duckdb_version
            )
        else:
    ...
    async def analyze_core_extensions_historical(
        self, duckdb_version: str, cutoff_date
    ) -> List[ExtensionInfo]:
        """Analyze core extensions as of a specific historical date."""
        logger.info(
            f"Starting historical core extensions analysis as of {cutoff_date.strftime('%Y-%m-%d')}"
        )

        # Get current core extensions first
        current_extensions = (
            await self.core_analyzer.analyze_with_platform_availability(duckdb_version)
        )
            ...
            if hasattr(ext, "last_push_date") and ext.last_push_date:
                ...
                try:
                    ...
                    if push_date.replace(tzinfo=None) <= cutoff_date:
                        ...
                        logger.debug(
                            f"Core extension '{ext.name}' was active before {cutoff_date.strftime('%Y-%m-%d')} (last activity: {push_date.strftime('%Y-%m-%d')})"
                        )
                    else:
                        # Extension became active after cutoff - exclude it
                        logger.info(
                            f"Core extension '{ext.name}' was NOT active as of {cutoff_date.strftime('%Y-%m-%d')} (first activity: {push_date.strftime('%Y-%m-%d')})"
                        )
                except Exception as e:
                    logger.warning(
                        f"Could not parse date for core extension '{ext.name}': {e}"
                    )
        ...
        logger.info(
            f"Historical core analysis: {len(historical_extensions)} of {len(current_extensions)} core extensions were active as of {cutoff_date.strftime('%Y-%m-%d')}"
        )
        return historical_extensions

    async def analyze_community_extensions_historical(
        self, cutoff_date
    ) -> List[ExtensionInfo]:
        """Analyze community extensions as of a specific historical date."""
        logger.info(
            f"Starting historical community extensions analysis as of {cutoff_date.strftime('%Y-%m-%d')}"
        )
            ...
            if hasattr(ext, "last_push_date") and ext.last_push_date:
                ...
                try:
                    ...
                    if push_date.replace(tzinfo=None) <= cutoff_date:
                        historical_extensions.append(ext)
                        logger.debug(
                            f"Community extension '{ext.name}' was active before {cutoff_date.strftime('%Y-%m-%d')} (last activity: {push_date.strftime('%Y-%m-%d')})"
                        )
                    else:
                        # Extension became active after cutoff - exclude it
                        logger.info(
                            f"Community extension '{ext.name}' was NOT active as of {cutoff_date.strftime('%Y-%m-%d')} (first activity: {push_date.strftime('%Y-%m-%d')})"
                        )
                except Exception as e:
                    logger.warning(
                        f"Could not parse date for community extension '{ext.name}': {e}"
                    )
                    # If we can't parse the date, include it to be safe
                    historical_extensions.append(ext)
            else:
        ...
        logger.info(
            f"Historical community analysis: {len(historical_extensions)} of {len(current_extensions)} community extensions were active as of {cutoff_date.strftime('%Y-%m-%d')}"
        )
        return historical_extensions
    ...
    async def analyze_full(self) -> AnalysisResult:
        ...
        async with httpx.AsyncClient() as client:
            ...
            (
                duckdb_version,
                duckdb_release_date,
            ) = await self.github_client.get_latest_duckdb_release(client)
            logger.info(
                f"Found latest DuckDB release: {duckdb_version} (published {duckdb_release_date.strftime('%Y-%m-%d')})"
            )
            ...
            if self.config.enable_issues_analysis:
                all_extension_names = [
                    ext.name for ext in core_extensions + community_extensions
                ]
            ...
            logger.info(
                f"ANALYSIS SUMMARY: DuckDB {duckdb_version} | Core: {len(core_extensions)} | Community: {len(community_extensions)} | Total: {len(core_extensions) + len(community_extensions)}"
            )
    ...
    async def validate_extension_urls(
        ...
        for ext in extensions:
            ...
            if hasattr(ext, "repository") and ext.repository:
                if ext.repository.startswith("http"):
                ...
                elif "/" in ext.repository and not ext.repository.startswith(
                    "integrated_core"
                ):
                    ...
                    if ext.repository != "duckdb/duckdb":
                        standard_urls_to_validate[f"{ext.name}_repository"] = (
                            f"https://github.com/{ext.repository}"
                        )
            ...
            if docs_url:
                docs_urls_to_validate[f"{ext.name}_documentation"] = (
                    docs_url,
                    ext.name,
                )
            ...
            if hasattr(ext, "metadata") and ext.metadata:
                ...
                if (
                    isinstance(ext.metadata, dict)
                    and "external_repository" in ext.metadata
                ):
                    repo_url = (
                        f"https://github.com/{ext.metadata['external_repository']}"
                    )
        ...
        if standard_urls_to_validate:
            standard_results = await self.url_validator.validate_urls_batch(
                standard_urls_to_validate
            )
        ...
        if docs_urls_to_validate:
            docs_results = await self.url_validator.validate_urls_with_content_batch(
                docs_urls_to_validate
            )
            validation_results.update(docs_results)
            logger.info(
                f"Validated {len(docs_urls_to_validate)} documentation URLs with content checking"
            )
    ...
    def _normalize_validation_results(
        ...
        for key, result in validation_results.items():
            ...
            if result.get("is_valid", False):
                ...
                if (
                    content_validation == "likely_wrong"
                    or extension_name_found is False
                ):
    ...
    async def analyze_full_historical(self, as_of_date: str) -> AnalysisResult:
        ...
        logger.info(
            f"Starting historical DuckDB extensions analysis as of {as_of_date}"
        )
        ...
        try:
            ...
            raise ValueError(
                f"Invalid date format '{as_of_date}'. Use YYYY-MM-DD format."
            )
        ...
        async with httpx.AsyncClient() as client:
            ...
            logger.info(
                f"Using DuckDB release: {duckdb_version} for historical context"
            )

            # Analyze core extensions with historical context
            core_extensions = await self.analyze_core_extensions_historical(
                duckdb_version, cutoff_date
            )
            logger.info(
                f"Analyzed {len(core_extensions)} core extensions as of {as_of_date}"
            )

            # Analyze community extensions with historical context
            community_extensions = await self.analyze_community_extensions_historical(
                cutoff_date
            )
            logger.info(
                f"Analyzed {len(community_extensions)} community extensions as of {as_of_date}"
            )
            ...
            logger.info(
                f"HISTORICAL ANALYSIS SUMMARY ({as_of_date}): DuckDB {duckdb_version} | Core: {len(core_extensions)} | Community: {len(community_extensions)} | Total: {len(core_extensions) + len(community_extensions)}"
            )
    ...
    async def generate_reports(
        self, analysis_result: AnalysisResult, formats: List[str] = None
    ) -> Dict[str, str]:
        ...
        for format_type in formats:
            try:
                if format_type.lower() == "markdown":
                    ...
                    content = await self.report_generator.generate_markdown_template(
                        analysis_result
                    )
                    ...
                    filepath = await self.report_generator.generate_excel(
                        analysis_result
                    )
                    results[format_type] = filepath

                elif format_type.lower() == "url_validation_csv":
                    filepath = await self.report_generator.generate_url_validation_csv(
                        analysis_result
                    )
                    if filepath:
                        results[format_type] = filepath
    ...
    async def run_analysis_mode(self, mode: str = "full") -> AnalysisResult:
        ...
        if mode == "core":
            ...
            return AnalysisResult(
                core_extensions=[], community_extensions=community_extensions
            )
    ...
    async def run_installation_tests(
        self, core_extensions: List, community_extensions: List
    ) -> List[Any]:
        ...
        logger.info(
            f"Running installation tests for {len(extensions_to_test)} extensions: {extensions_to_test}"
        )
        ...
        try:
            ...
            logger.info(
                f"Installation testing completed. {len(installation_results)} results."
            )
    ...
    async def run_report_generation(
        self, formats: List[str] = None, as_of_date: str = None
    ) -> Dict[str, str]:
