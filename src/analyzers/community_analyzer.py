...
class CommunityExtensionAnalyzer(BaseAnalyzer):
    ...
    def __init__(
        ...
        cache_dir = (
            config.cache_dir / "deprecation_detector"
            if hasattr(config, "cache_dir")
            else None
        )
        # Convert hours to days for cache TTL (cache_hours / 24)
        cache_days = cache_hours / 24
        deprecation_cache = RepositoryCache(
            cache_dir=cache_dir, cache_days=cache_days, enabled=True
        )
    ...
    async def get_duckdb_test_versions(
        self, client: httpx.AsyncClient
    ) -> tuple[str, str]:
        ...
        try:
            ...
            response = await client.get(
                ...
                headers={
                    ...
                    "Authorization": f"token {self.github_client.github_token}"
            ...
            if response.status_code == 200:
                ...
                stable_releases = [
                    r for r in releases if not r.get("prerelease", False)
                ]

                if len(stable_releases) >= 2:
                    current_version = stable_releases[0]["tag_name"]  # Most recent
                    previous_version = stable_releases[1][
                        "tag_name"
                    ]  # Second most recent
                    ...
                    logger.info(
                        f"DuckDB versions for testing: current={current_version}, previous={previous_version}"
                    )
                    return self._duckdb_versions_cache
        ...
        logger.info(
            f"Using fallback DuckDB versions: current={current_fallback}, previous={previous_fallback}"
        )
    ...
    async def get_official_extensions_list(self, client: httpx.AsyncClient) -> set[str]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"Found {len(official_extensions)} official extensions for current DuckDB version"
                )
    ...
    def determine_extension_compatibility_status(
        ...
        ext_name: str,
        is_official: bool,
        install_v14_success: bool,
        install_v13_success: bool,
        deprecation_score: float,
        metadata_deprecated: bool,
    ) -> tuple[str, str]:
        ...
        if install_v14_success:
            ...
            if deprecation_score >= 7.0:
                ...
                return (
                    ...
                    "installation_failed_medium_deprecation",
                )
            else:
    ...
    async def analyze_extension_deprecation(
        ...
        ext_name: str,
        repo_url: str,
        ce_metadata: Optional[Dict] = None,
    ) -> Optional[Dict]:
        ...
        try:
            ...
            if deprecation_result.get("status") == "analyzed":
                return {
                    "deprecation_score": deprecation_result.get(
                        "deprecation_score", 0.0
                    ),
                    "recommendation": deprecation_result.get(
                        "recommendation", "ACTIVE"
                    ),
                    "deprecation_indicators": deprecation_result.get(
                        "deprecation_indicators", []
                    ),
                    "warning_indicators": deprecation_result.get(
                        "warning_indicators", []
                    ),
                    "active_indicators": deprecation_result.get(
                        "active_indicators", []
                    ),
                    "repository_archived": deprecation_result.get(
                ...
                logger.debug(
                    f"Deprecation analysis failed for {ext_name}: {deprecation_result.get('error', 'Unknown error')}"
                )
                return None
    ...
    async def analyze_community_extensions(
        ...
        if self.enable_compatibility_testing:
            current_version, previous_version = await self.get_duckdb_test_versions(
                client
            )
            logger.info(
                f"Compatibility testing enabled for DuckDB {current_version} and {previous_version}"
            )
                    ...
                    if repo_info:
                        ...
                        if metadata:
                            ext_info["ce_metadata"] = {
                                "official_description": metadata.get(
                                    "extension", {}
                                ).get("description"),
                                "version": metadata.get("extension", {}).get("version"),
                                "language": metadata.get("extension", {}).get(
                                    "language"
                                ),
                                "maintainers": metadata.get("extension", {}).get(
                                    "maintainers", []
                                ),
                                "license": metadata.get("extension", {}).get("license"),
                                "build_system": metadata.get("extension", {}).get(
                                    "build"
                                ),
                        ...
                        install_v14_success = (
                            is_official  # Temporary: assume official = v1.4 compatible
                        )
                        install_v13_success = (
                            True  # Temporary: assume all work with v1.3
                        )

                        # Get deprecation score
                        deprecation_score = (
                            deprecation_analysis.get("deprecation_score", 0.0)
                            if deprecation_analysis
                            else 0.0
                        )
                        ...
                        if metadata_helper.is_deprecated_extension(ext):
                            ...
                            ext_info["deprecated_info"] = (
                                metadata_helper.get_deprecated_extension_info(ext)
                            )
                            ...
                            ext_info["review_info"] = (
                                metadata_helper.get_review_required_extension_info(ext)
                            )
                            ...
                            ext_info["template_info"] = (
                                metadata_helper.get_template_extension_info(ext)
                            )
                            ...
                            status, compatibility_status = (
                                self.determine_extension_compatibility_status(
                                    ext,
                                    is_official,
                                    install_v14_success,
                                    install_v13_success,
                                    deprecation_score,
                                    False,  # Not manually deprecated
                                )
                            )
        ...
        stats = {
            ...
            "deprecated": sum(
                1 for ext in extension_data if ext["status"] == "⚠️ Deprecated"
            ),
            "review_required": sum(
                1 for ext in extension_data if ext["status"] == "⚠️ Review Required"
            ),
            "likely_deprecated": sum(
                1 for ext in extension_data if ext["status"] == "❌ Likely Deprecated"
            ),
            "version_incompatible": sum(
                1
                for ext in extension_data
                if ext["status"] == "❌ Version Incompatible"
            ),
            "installation_failed": sum(
                1 for ext in extension_data if ext["status"] == "❌ Installation Failed"
            ),
            "templates": sum(
                1 for ext in extension_data if ext["status"] == "🔧 Template"
            ),
            "discontinued": sum(
                1 for ext in extension_data if ext["status"] == "🔴 Discontinued"
            ),
            "errors": sum(1 for ext in extension_data if ext["status"] == "❌ Error"),
            "official_extensions": sum(
                1 for ext in extension_data if ext.get("is_official", False)
            ),
            "unofficial_extensions": sum(
                1 for ext in extension_data if not ext.get("is_official", True)
            ),
            "auto_deprecation_detected": sum(
                1
                for ext in extension_data
                if ext.get("auto_deprecation_detected", False)
            ),
            "auto_review_recommended": sum(
                1 for ext in extension_data if ext.get("auto_review_recommended", False)
            ),
        }
        ...
        async with httpx.AsyncClient() as client:
            ...
            for ext_data in extension_data:
                ...
                ext_info.metadata = {
                    ...
                    "auto_deprecation_detected": ext_data.get(
                        "auto_deprecation_detected", False
                    ),
                    "auto_review_recommended": ext_data.get(
                        "auto_review_recommended", False
                    ),
                    "is_official": ext_data.get("is_official", False),
                    "compatibility_status": ext_data.get(
