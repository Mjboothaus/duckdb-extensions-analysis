...
class TemplateEngine:
    ...
    def __init__(self, config: Config, templates_dir: Optional[Path] = None):
        ...
        logger.info(
            f"Template engine initialized with templates from: {self.templates_dir}"
        )
    ...
    def _filter_strftime(
        self, date: Union[datetime, str], format_str: str = "%Y-%m-%d"
    ) -> str:
    ...
    def _filter_relative_date(self, date_or_days: Union[datetime, int, str]) -> str:
        ...
        thresholds = (
            self.format_config.get("formatting", {})
            .get("dates", {})
            .get(
                "relative_thresholds",
                [
    ...
    def _filter_datetime_with_relative(
        self, days_ago: Union[int, str], last_push: Optional[str] = None
    ) -> str:
    ...
    def _filter_status_badge(self, status: str) -> str:
        ...
        status_mapping = self.format_config.get("formatting", {}).get(
            "status",
            {
                "ongoing": "🟢 Ongoing",
    ...
    def _filter_github_link(
        self, repository: str, validation_results: Dict = None, ext_name: str = ""
    ) -> str:
    ...
    def _filter_truncate(
        self, text: str, length: int = 100, suffix: str = "..."
    ) -> str:
    ...
    def _filter_validated_link(
        ...
        if validation_results:
            ...
            url_key_patterns = [
                f"{ext_name}_repository",
                f"{ext_name}_documentation",
                f"{ext_name}_external_repo",
            ]
            validation_result = None

            for pattern in url_key_patterns:
                if (
                    pattern in validation_results
                    and validation_results[pattern].get("url") == url
                ):
    ...
    def prepare_template_data(self, analysis_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        template_data = {
            "report": {
                "name": self.report_config.get("reports", {})
                .get("full_analysis", {})
                .get("name", "DuckDB Extensions Analysis"),
                "description": self.report_config.get("reports", {})
                .get("full_analysis", {})
                .get("description", ""),
            ...
            "third_party_extensions": self._prepare_extensions_data(
    ...
    def _prepare_extensions_data(
        ...
        for ext in extensions:
            ...
            prepared_ext = {
                ...
                "last_activity": ext.get("last_push_days")
                or ext.get("days_ago", 0),  # Handle both field names
    ...
    def _count_recent_activity(
        self, extensions: List[Dict[str, Any]], days_threshold: int
    ) -> int:
        ...
        for ext in extensions:
            ...
            if (
                isinstance(last_push_days, (int, float))
                and last_push_days <= days_threshold
            ):
                count += 1
        return count
        ...
        template_data["report"].update(
            {
                ...
                "description": report_config.get(
                    "description", template_data["report"]["description"]
                ),
            }
        )
    ...
    def render_component(
        self, component_name: str, template_data: Dict[str, Any]
    ) -> str:
        """
        ...
        template_file = component_config.get(
            "template", f"components/{component_name}.md.j2"
        )
