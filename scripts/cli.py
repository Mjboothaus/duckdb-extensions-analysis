...
def status_community(extensions, max_age, as_of_date, output_json):
    ...
    if not extensions:
        click.echo(
            "❌ No extensions specified. Use: status community <extension1> <extension2> ..."
        )
        return

    asyncio.run(
        _check_community_extensions_status(
            list(extensions), max_age, as_of_date, output_json
        )
    )
...
def status_all(
    max_age_core, max_age_community, as_of_date, output_json, community_extensions
):
    ...
    """
    asyncio.run(
        _check_all_extensions_status(
            max_age_core,
            max_age_community,
            as_of_date,
            list(community_extensions),
            output_json,
        )
    )
...
async def _run_analysis(
    mode: str, cache_hours: Optional[int], with_compatibility_testing: bool = False
):
    ...
    if with_compatibility_testing:
        logger.info(
            "Enabling extension compatibility testing (--with-compatibility-testing flag)"
        )
...
async def _check_core_extensions_status(
    max_age_days: int, as_of_date: Optional[str], output_json: bool
):
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            (
                duckdb_version,
                duckdb_release_date,
            ) = await orchestrator.github_client.get_latest_duckdb_release(client)

            if not duckdb_release_date:
                if output_json:
                    click.echo(
                        json.dumps({"error": "Could not fetch DuckDB release date"})
                    )
                else:
            ...
            if output_json:
                ...
                click.echo(
                    f"   Release Date: {duckdb_release_date.strftime('%Y-%m-%d')} ({age_desc})"
                )
                if as_of_date:
                    click.echo(
                        f"   Reference Date: {reference_date.strftime('%Y-%m-%d')} (as-of-date specified)"
                    )
                click.echo(f"   Max Age: {max_age_days} days")

                if not is_fresh:
                    click.echo(
                        f"   ⚠️  DuckDB release is {release_age_days - max_age_days} days older than threshold"
                    )
                    click.echo("   📝 Consider updating to newer core extensions")
...
async def _check_community_extensions_status(
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            extensions_list = (
                await orchestrator.github_client.get_community_extensions_list(client)
            )
            ...
            for ext_name in extension_names:
                ...
                metadata = await orchestrator.community_analyzer.get_extension_metadata(
                    client, ext_name
                )
                if (
                    not metadata
                    or "repo" not in metadata
                    or "github" not in metadata["repo"]
                ):
                    result = {
                        "name": ext_name,
                        "status": "no_repo",
                ...
                repo_info = await orchestrator.community_analyzer.get_repository_info(
                    client, repo
                )
        ...
        if output_json:
            output = {
                ...
                "summary": {
                    ...
                    "outdated": sum(
                        1 for r in results if r.get("status") == "outdated"
                    ),
            ...
            for result in results:
                ...
                if "error" in result:
                    ...
                    if as_of_date:
                        ...
                        age_desc = (
                            f"{age} days{' ago' if age >= 0 else ' in the future'}"
                        )
...
async def _check_all_extensions_status(
    max_age_core: int,
    max_age_community: int,
    as_of_date: Optional[str],
    community_extensions: List[str],
    output_json: bool,
):
    ...
    if output_json:
        ...
        if community_extensions:
            try:
                f = io.StringIO()
                with contextlib.redirect_stdout(f):
                    await _check_community_extensions_status(
                        community_extensions, max_age_community, as_of_date, True
                    )
                community_json = f.getvalue()
                community_result = (
                    json.loads(community_json) if community_json.strip() else {}
                )
            except Exception as e:
        ...
        if community_extensions:
            ...
            await _check_community_extensions_status(
                community_extensions, max_age_community, as_of_date, False
            )
