...
def check_for_penalty():
    ...
    try:
        response = httpx.get(
            "https://api.github.com/rate_limit", headers=config.headers, timeout=10
        )
        data = response.json()
        ...
        print(
            f"  Resets at: {datetime.fromtimestamp(core['reset']).strftime('%Y-%m-%d %H:%M:%S')}"
        )
    ...
    test_repos = ["duckdb/duckdb", "duckdb/community-extensions", "query-farm/a5"]
