...
def query_database(db_path: str = "data/extensions.duckdb"):
    ...
    for row in result:
        print(f"📅 {row[0]} | DuckDB: {row[1]} | Script: {row[2]} | Core: {row[3]} | Community: {row[4]} | Featured: {row[5]}")
        if row[6]:
            print(f"   💬 {row[6]}")
    ...
    for row in result:
        ...
        desc = row[3][:50] + "..." if row[3] and len(row[3]) > 50 else (row[3] or "No description")
    ...
    for row in result:
        print(f"Version {row[0]}: {row[1]} extensions ({row[2]} featured) | {row[3]} to {row[4]}")
...
def simulate_historical_backfill(db_path: str = "data/extensions.duckdb"):
    ...
    for i, (date, version) in enumerate(zip(older_dates, versions)):
        ...
        print(f"📅 Added historical run: {date.strftime('%Y-%m-%d')} | {version} | {community_count} extensions")
    ...
    print(f"✅ Historical back-fill completed! Added data for {len(older_dates)} time periods")
    conn.close()
