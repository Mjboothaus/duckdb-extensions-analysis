...
CompatibilityStatus = Literal[
    "compatible", "incompatible", "not_available", "not_exists", "unknown"
]
...
class MockVersionCompatibilityAnalyzer:
    ...
    async def generate_compatibility_timeline(
        ...
        print(
            f"🔍 Analyzing {len(extension_names)} extensions across {len(duckdb_versions)} DuckDB versions..."
        )
        ...
        for ext_name in extension_names:
            ...
            for version in duckdb_versions:
                ...
                mock_data = self.mock_data.get(ext_name, {}).get(
                    version,
                    {
...
def format_results(
    timeline: Dict[str, List[MockVersionCompatibilityInfo]], format_type: str = "table"
) -> str:
    ...
    if format_type == "table":
        ...
        for ext_name, results in timeline.items():
            ...
            for result in results:
                ...
                output.append(
                    f"  DuckDB v{result.duckdb_version}: {status_emoji} {result.compatibility_status.upper()}"
                )
                output.append(
                    f"    Repository: {'✓' if result.exists_in_repo else '✗'} exists"
                )
                output.append(
                    f"    Download:   {'✓' if result.available_for_download else '✗'} available"
                )

                if result.functional_compatible is not None:
                    output.append(
                        f"    Functional: {'✓' if result.functional_compatible else '✗'} working"
                    )
                else:
                    output.append("    Functional: ? untested")
...
def analyze_compatibility_trends(
    timeline: Dict[str, List[MockVersionCompatibilityInfo]],
) -> Dict:
    ...
    for ext_name, results in timeline.items():
        ...
        for result in sorted_results:
            ...
            if (
                ...
                and first_working_version is None
            ):
        ...
        trends["extension_maturity"][ext_name] = {
            ...
            "has_maturation_delay": (
                first_repo_appearance
                and first_working_version
                and first_repo_appearance != first_working_version
            ),
        }
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Demo DuckDB Extension Version Compatibility Analysis"
    )
    parser.add_argument(
        "--extensions",
        default="ui,json,parquet",
        help="Comma-separated extension names (default: ui,json,parquet)",
    )
    parser.add_argument(
        "--versions",
        default="1.3.2,1.4.0,1.4.1,1.5.0",
    ...
    for ext_name, maturity in trends["extension_maturity"].items():
        ...
        if maturity["has_maturation_delay"]:
            print("    ⚠️  Had maturation delay (existed before working)")
        elif maturity["first_repo_appearance"] == maturity["first_working_version"]:
            print("    ✅ Immediate compatibility")
        print()

    print("📊 Version-by-Version Analysis:")
    for version, data in trends["version_analysis"].items():
        compatible_pct = (
            (data["compatible_count"] / data["total_count"] * 100)
            if data["total_count"] > 0
            else 0
        )
        print(
            f"  DuckDB v{version}: {data['compatible_count']}/{data['total_count']} compatible ({compatible_pct:.1f}%)"
        )
