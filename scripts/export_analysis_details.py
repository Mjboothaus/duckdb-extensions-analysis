...
async def export_detailed_analysis():
    ...
    try:
            ...
            detailed_data.append(
                {
                    ...
                    "last_push_date": ext.last_push.isoformat()
                    if ext.last_push
                    else "Unknown",
        ...
        for ext in analysis_result.community_extensions:
            repo_accessible: str
            if ext.repository and hasattr(ext, "repository_error"):
                repo_accessible = "No" if ext.repository_error else "Yes"
            elif ext.repository:
                repo_accessible = "Yes"
            else:
                repo_accessible = "No"
            ...
            detailed_data.append(
                {
                    ...
                    "description": ext.description,
                    "docs_url": ext.docs_url,
                    "activity_level": _classify_activity(
                        ext.last_activity, "community"
                    ),
        ...
        print(f"📊 Total extensions analysed: {len(detailed_data)}")

        # Show summary by status
        status_counts: dict[str, int] = {}
        for ext_data in detailed_data:
            status = str(ext_data["status"])
...
def _get_status_reasoning(ext, ext_type):
    ...
    if ext.last_activity is not None:
        if ext_type == "core":
            if ext.last_activity <= 7:
                reasons.append(
                    f"Recent activity: {ext.last_activity} days ago (≤7 day threshold for core)"
                )
            elif ext.last_activity <= 30:
                reasons.append(
                    f"Moderate activity: {ext.last_activity} days ago (within 30 days)"
                )
            else:
            ...
            if ext.last_activity <= 30:
                reasons.append(
                    f"Recent activity: {ext.last_activity} days ago (≤30 day threshold)"
                )
            elif ext.last_activity <= 90:
                reasons.append(
                    f"Moderate activity: {ext.last_activity} days ago (within 90 days)"
                )
            elif ext.last_activity <= 365:
                reasons.append(
                    f"Low activity: {ext.last_activity} days ago (within 1 year)"
                )
            else:
                reasons.append(
                    f"Very low activity: {ext.last_activity} days ago (>1 year)"
                )
    else:
    ...
    if hasattr(ext, "deprecation_reasons") and ext.deprecation_reasons:
        reasons.append(
            f"Deprecation indicators: {', '.join(ext.deprecation_reasons[:3])}"
        )
