...
class ExtensionAnalyzer:
    ...
    def get_core_extensions(self) -> List[Dict]:
        ...
        logger.info(f"Found {len(extensions)} core extensions from DuckDB documentation")
        return extensions
    
    async def get_core_extension_github_info(self, client: httpx.AsyncClient, ext_name: str) -> Optional[Dict]:
        ...
        try:
            ...
            commits = await fetch_github_api_cached(client, commits_url, cache_hours=self.cache_hours)
            ...
            logger.debug(f"Could not get GitHub info for core extension {ext_name}: {e}")
    ...
    async def get_featured_extensions(self, client: httpx.AsyncClient) -> set[str]:
        ...
        try:
                ...
                try:
                    ...
                    for element in soup.find_all(['code', 'strong', 'b']):
                        ...
                        if (text and 
                            len(text) > 2 and 
                            len(text) < 30 and 
                            text.replace('_', '').replace('-', '').replace('.', '').isalnum() and
            ...
            if len(featured_extensions) < 5:
                logger.warning("Could not detect featured extensions dynamically, using curated list")
                featured_extensions = {
            ...
            featured_extensions = featured_extensions.intersection(set(name.lower() for name in known_extensions))
    ...
    def get_extension_links(self, ext_name: str, repo: str = None) -> Dict[str, str]:
        ...
        links['community_repo'] = f"https://github.com/duckdb/community-extensions/tree/main/extensions/{ext_name}"
        ...
        links['install'] = f"https://duckdb.org/docs/extensions/community_extensions.html#{ext_name}"
        
        return links
    
    def improve_description(self, ext_name: str, description: str, repo: str = None, topics: List[str] = None) -> str:
    ...
    async def get_latest_duckdb_release(self, client: httpx.AsyncClient) -> Tuple[str, datetime]:
        ...
        try:
            release_data = await fetch_github_api_cached(client, releases_url, cache_hours=self.cache_hours)
            ...
            logger.info(f"Found latest DuckDB release: {version} (published {release_date.strftime('%Y-%m-%d')})")
            ...
            logger.warning(f"Using fallback values: {fallback_version} ({fallback_date.strftime('%Y-%m-%d')})")
    ...
    def create_database_schema(self, conn: duckdb.DuckDBPyConnection) -> None:
        ...
        conn.execute("CREATE INDEX IF NOT EXISTS idx_core_ext_hist_name_date ON core_extensions_history(name, analysis_date)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_community_ext_hist_name_date ON community_extensions_history(name, analysis_date)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_core_ext_hist_duckdb_version ON core_extensions_history(duckdb_version)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_community_ext_hist_duckdb_version ON community_extensions_history(duckdb_version)")
        
        logger.info("Database schema created successfully with historical tracking")
    
    async def save_to_database(self, extension_data: List[Dict], db_path: str = "data/extensions.duckdb") -> None:
        ...
        try:
            ...
            async with httpx.AsyncClient() as temp_client:
                duckdb_version, duckdb_release_date = await self.get_latest_duckdb_release(temp_client)
            ...
            featured_count = sum(1 for ext in extension_data if ext.get('featured', False))
            conn.execute("""
            ...
            for ext in core_extensions:
                ...
                github_info = None
                async with httpx.AsyncClient() as temp_client:
                    github_info = await self.get_core_extension_github_info(temp_client, ext['name'])
                ...
                if github_info:
                    last_commit_date = datetime.fromisoformat(github_info['last_commit_date'].rstrip('Z'))
            ...
            for ext_info in extension_data:
                ...
                if not ext_info["error"] and ext_info["repo_info"]:
                    ...
                    if repo_info["last_push"]:
                        last_push_date = datetime.fromisoformat(repo_info["last_push"].rstrip('Z'))
                    
                    created_at = None
                    if repo_info["created_at"]:
                        created_at = datetime.fromisoformat(repo_info["created_at"].rstrip('Z'))
                    
                    updated_at = None
                    if repo_info["updated_at"]:
                        updated_at = datetime.fromisoformat(repo_info["updated_at"].rstrip('Z'))
            ...
            logger.info(f"Successfully saved {len(core_extensions)} core extensions and {len(extension_data)} community extensions to database")
    ...
    async def get_community_extensions(self, client: httpx.AsyncClient) -> List[str]:
        ...
        contents = await fetch_github_api_cached(client, contents_url, cache_hours=self.cache_hours)
    ...
    async def get_extension_metadata(self, client: httpx.AsyncClient, ext_name: str) -> Optional[Dict]:
        ...
        try:
            metadata_raw = await fetch_github_api_cached(client, metadata_url, cache_hours=self.cache_hours)
    ...
    async def get_repository_info(self, client: httpx.AsyncClient, repo: str) -> Optional[Dict]:
        ...
        try:
            repo_data = await fetch_github_api_cached(client, repo_url, cache_hours=self.cache_hours)
            return {
    ...
    async def analyze_community_extensions(self, client: httpx.AsyncClient) -> Tuple[List[Dict], Dict]:
        ...
        for ext in extensions:
            ...
            ext_info = {
                ...
                "status": "❌ Error",
                "last_push_days": None,
                "featured": ext.lower() in featured_extensions,  # Mark as featured if in the list
    ...
    async def print_core_analysis(self, client: httpx.AsyncClient):
        ...
        # Get DuckDB release info
        duckdb_version, duckdb_release_date = await self.get_latest_duckdb_release(client)
        ...
        for ext in extensions:
            ...
            if github_info:
                last_commit_days = self.calculate_days_ago(github_info['last_commit_date'])
                logger.info(f"{ext['name']} (Core, Stage: {stage}): Ongoing | Last commit: {last_commit_days} days ago")
            else:
    ...
    async def generate_markdown_report(self, client: httpx.AsyncClient, extension_data: List[Dict], stats: Dict) -> str:
    ...
    async def save_csv_report(self, client: httpx.AsyncClient, extension_data: List[Dict], stats: Dict) -> Path:
        ...
        for ext_info in extension_data:
            ...
            if ext_info["error"]:
                ...
                rows.append({
                    ...
                    'Topics': ','.join(repo_info["topics"]) if repo_info["topics"] else '',
                })
    ...
    async def save_excel_report(self, client: httpx.AsyncClient, extension_data: List[Dict], stats: Dict) -> Path:
        ...
        for ext_info in extension_data:
            ...
            if ext_info["error"]:
                ...
                community_rows.append({
                    ...
                    'Repository': repo,
                    'Status': ext_info["status"],
                    'Last Push': f"{ext_info['last_push_days']} days ago" if ext_info['last_push_days'] is not None else 'Unknown',
                    'Stars': repo_info["stars"],
                    'Language': repo_info["language"] or 'N/A',
                    'Description': repo_info["description"] or 'No description',
                    'Homepage': repo_info["homepage"] or 'N/A',
        ...
        with pd.ExcelWriter(excel_file, engine='openpyxl') as writer:
            ...
            community_df.to_excel(writer, sheet_name='Community Extensions', index=False)
            
            # Summary sheet
            summary_data = [
                ['Metric', 'Core Extensions', 'Community Extensions', 'Total'],
                ['Total Count', len(core_extensions), stats['total'], len(core_extensions) + stats['total']],
                ['Active Count', len(core_extensions), stats['active'], len(core_extensions) + stats['active']],
...
Examples:
    ...
    cache_hours = 0 if args.no_cache else 1  # Use 0 hours (no caching) if --no-cache is specified
    if args.no_cache:
    ...
    if args.mode == "core":
        ...
        async with httpx.AsyncClient() as client:
            ...
            report_content = await analyzer.generate_markdown_report(client, extension_data, stats)
            ...
            if args.excel:
                excel_file = await analyzer.save_excel_report(client, extension_data, stats)
                logger.info(f"Excel report saved: {excel_file}")
