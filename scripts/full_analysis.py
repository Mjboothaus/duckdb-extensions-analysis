...
async def get_extension_metadata(client, ext_name):
    metadata_url = f"{GITHUB_API_BASE}/repos/{COMMUNITY_REPO}/contents/extensions/{ext_name}/metadata.toml"
    try:
        ...
        metadata = toml.loads(metadata_content)
        return metadata
    except httpx.HTTPStatusError as e:
        logger.warning(f"Metadata not found for {ext_name}: {e}")
        return None
...
async def check_community_status(client):
    ...
    for ext in extensions:
        ...
        if not metadata or "repository" not in metadata:
            logger.warning(f"No repository found for {ext}. Skipping.")
            continue
        repo = metadata["repository"].replace("https://github.com/", "")
