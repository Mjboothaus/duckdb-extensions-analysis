...
def check_core_status():
    ...
    for ext in extensions:
        ...
        logger.info(
            f"{ext['name']} (Core, Stage: {ext['stage']}): {status} | Last updated: {last_push} ({lag_days} days ago in {DUCKDB_VERSION})"
        )
...
async def check_community_status(client):
    ...
    for ext in extensions:
        ...
        try:
            ...
            lag_days = (
                (CURRENT_DATE - last_push_date).days if last_push_date else "Unknown"
            )
            status = "Discontinued" if archived else "Ongoing"
            logger.info(
                f"{ext} (Repo: {repo}): {status} | Last push: {last_push_date} ({lag_days} days ago)"
            )
