...
class ReleaseHistoryValidator:
            ...
            if in_table and (
                ...
                if len(parts) >= 5:  # Minimum columns needed
                    ...
                    if version_match:
                        ...
                        has_blog_link = (
                            blog_info.startswith("[")
                            and "](" in blog_info
                            and "http" in blog_info
                        )
    ...
    async def validate_releases(self) -> Tuple[List[str], List[str], List[str]]:
        ...
        try:
            ...
            for gh_release in github_releases[:10]:  # Check top 10
                ...
                if version in template_releases:
                    ...
                    if is_upcoming and not is_prerelease:
                        errors.append(
                            f"❌ {version} is marked as upcoming but is already released ({gh_date})"
                        )

                    # Check date accuracy
                    if template_date != gh_date and not is_upcoming:
                        warnings.append(
                            f"⚠️  {version} date mismatch: template={template_date}, GitHub={gh_date}"
                        )
                    ...
                    if (
                        ...
                        warnings.append(
                            f"⚠️  {version} (major release) missing blog link"
                        )
                    ...
                    if not is_prerelease:
                        errors.append(
                            f"❌ {version} (released {gh_date}) missing from template"
                        )
                    else:
            ...
            for version in template_releases:
                if (
                    version not in gh_versions
                    and not template_releases[version]["is_upcoming"]
                ):
                    warnings.append(
                        f"⚠️  Template has {version} but not found in recent GitHub releases"
                    )
    ...
    async def run_validation(self) -> bool:
        ...
        if errors:
            ...
            logger.error(
                "🔧 Action needed: Update templates/components/duckdb_release_info.md.j2"
            )
            return False
        elif warnings:
