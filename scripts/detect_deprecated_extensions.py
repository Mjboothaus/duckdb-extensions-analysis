...
class DeprecationDetector:
    ...
    def __init__(
        ...
        self.cache = cache or RepositoryCache(
            enabled=False
        )  # Default to no cache unless provided
    ...
    async def analyze_extension(
        ...
        result = {
            ...
            "official_version": metadata.get("extension", {}).get("version", "")
            if metadata
            else "",
        ...
        try:
            ...
            if repo_info:
                ...
                if result["description"]:
                    ...
                    result["warning_indicators"].extend(
                        self._find_indicators(
                            result["description"], WARNING_KEYWORDS, "repo_description"
                        )
                    )
                    result["active_indicators"].extend(
                        self._find_indicators(
                            result["description"], ACTIVE_KEYWORDS, "repo_description"
                        )
                    )
            ...
            if readme_content:
                result["readme_content"] = readme_content[
                    :1000
                ]  # First 1000 chars for reference

                # Analyze README for indicators
                result["deprecation_indicators"].extend(
                    self._find_indicators(
                        readme_content, DEPRECATION_KEYWORDS, "readme"
                    )
                )
                result["warning_indicators"].extend(
    ...
    async def _get_repository_info(
        ...
        if self.github_api_client:
            try:
                ...
                return await self.github_api_client.get_repository_info(
                    client, repo_path
                )
            except Exception as e:
                logger.debug(
                    f"Error fetching repository info via GitHubAPIClient for {owner}/{repo}: {e}"
                )
                return None
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                repo_data = {
                    "_not_found": True
                }  # Cache the 404 to avoid repeated calls
            else:
                logger.warning(
                    f"Failed to fetch repo info for {owner}/{repo}: HTTP {response.status_code}"
                )
                return None
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if content.get("encoding") == "base64":
                        ...
                        readme_text = base64.b64decode(content["content"]).decode(
                            "utf-8", errors="ignore"
                        )
    ...
    def _find_indicators(
        self, text: str, keywords: List[str], source: str
    ) -> List[Dict]:
        ...
        for keyword in keywords:
            if keyword.lower() in text_lower:
                ...
                pattern = re.compile(
                    rf".{{0,50}}{re.escape(keyword.lower())}.{{0,50}}", re.IGNORECASE
                )
                matches = pattern.findall(text)

                for match in matches[:3]:  # Limit to first 3 matches
                    indicators.append(
                        {"keyword": keyword, "source": source, "context": match.strip()}
                    )
        ...
        if result["last_push"]:
            try:
                last_push = datetime.fromisoformat(
                    result["last_push"].replace("Z", "+00:00")
                )
                days_ago = (
                    datetime.now().replace(tzinfo=last_push.tzinfo) - last_push
                ).days
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        for ext_name, extension_info in extensions_data.items():
            try:
                ...
                result = await detector.analyze_extension(
                    client, ext_name, repo_url, metadata
                )
...
async def get_community_extensions_list(
    client: httpx.AsyncClient, detector: DeprecationDetector
) -> Dict[str, str]:
    ...
    if cached_data is not None:
        logger.info(
            f"Using cached community extensions list ({len(cached_data)} extensions)"
        )
        return cached_data
    ...
    try:
        ...
        if response.status_code != 200:
            logger.error(
                f"Failed to fetch community extensions list: HTTP {response.status_code}"
            )
            return {}
            ...
            if item["type"] == "dir":
                ...
                try:
                    ...
                    metadata_response = await client.get(
                        metadata_url, headers=detector.headers
                    )
                    ...
                    if metadata_response.status_code == 200:
                        ...
                        if metadata_content.get("encoding") == "base64":
                            ...
                            if github_repo:
                                ...
                                logger.debug(
                                    f"Extension {ext_name}: GitHub repo = {github_repo}, version = {metadata.get('extension', {}).get('version', 'unknown')}"
                                )
...
def format_results(results: List[Dict], format_type: str = "markdown") -> str:
    ...
    if format_type == "json":
        ...
        deprecated_count = sum(
            1 for r in results if "DEPRECATED" in r.get("recommendation", "")
        )
        likely_deprecated = sum(
            1 for r in results if "LIKELY DEPRECATED" in r.get("recommendation", "")
        )
        review_needed = sum(
            1
            for r in results
            if "REVIEW" in r.get("recommendation", "")
            or "POSSIBLY DEPRECATED" in r.get("recommendation", "")
        )
        ...
        sorted_results = sorted(
            results, key=lambda x: x.get("deprecation_score", 0), reverse=True
        )

        # High priority extensions (likely deprecated)
        high_priority = [
            r for r in sorted_results if r.get("deprecation_score", 0) >= 5.0
        ]
        if high_priority:
            ...
            for result in high_priority:
                ...
                output.append(
                    f"| {ext_name} | {repo_display} | {version} | {score:.1f} | {recommendation} | {last_push} |"
                )
        ...
        medium_priority = [
            r for r in sorted_results if 1.0 <= r.get("deprecation_score", 0) < 5.0
        ]
        if medium_priority:
            ...
            for result in medium_priority:
                ...
                output.append(
                    f"| {ext_name} | {repo_display} | {version} | {score:.1f} | {recommendation} |"
                )
        ...
        if high_priority:
            ...
            for result in high_priority:
                ...
                if result.get("official_description"):
                    output.append(
                        f"**Official Description:** {result.get('official_description')}\n"
                    )
                ...
                if deprecation_indicators:
                    ...
                    for indicator in deprecation_indicators[:5]:  # Limit to 5
                        ...
                        context = (
                            indicator.get("context", "")[:100] + "..."
                            if len(indicator.get("context", "")) > 100
                            else indicator.get("context", "")
                        )
...
async def main():
    ...
    parser.add_argument(
        "--cache-dir",
        help="Directory to store cached data (default: .cache/deprecation_detector)",
    )
    parser.add_argument(
        "--no-cache", action="store_true", help="Disable caching of repository data"
    )
    parser.add_argument(
        "--cache-days",
        type=int,
        default=7,
        help="Number of days to keep cached data (default: 7)",
    )
    ...
    if not github_token:
        ...
        logger.info(
            "Set GITHUB_TOKEN environment variable or use --github-token option for better API limits."
        )
    ...
    cache = RepositoryCache(
        cache_dir=cache_dir, cache_days=args.cache_days, enabled=cache_enabled
    )
    ...
    logger.success(
        f"Analysis complete! {total} extensions analyzed, {deprecated + likely_deprecated} flagged for review."
    )
